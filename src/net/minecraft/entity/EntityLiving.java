...
public class EntityLiving extends Entity {
    ...
    protected boolean sprintingToggled = false;
    ...
    public void moveEntityWithHeading(final float nya1, final float moveForward) {
        ...
        // 如果处于疾跑状态且在地面上且向前移动，增加速度
        if (this.isSprinting && moveForward > 0) {
            baseSpeed *= 1.3f; // 疾跑速度加成
        }
        ...
        if (this.handleWaterMovement()) {
            ...
            // 使用计算后的基础速度（包含疾跑加成）
            this.moveFlying(nya1, moveForward, baseSpeed);
            if (this.isOnLadder()) {
