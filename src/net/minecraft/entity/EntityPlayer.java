...
public class EntityPlayer extends EntityLiving {
    ...
    public void onLivingUpdate() {
        ...
        if (this.onGround && this.isSprinting) {
            if (this.worldObj.isBlockNormalCubeOrAir(MathHelper.floor_double(posX), (int) posY, MathHelper.floor_double(posZ))) {
                MinecraftRunner.minecraft.effectRenderer.addWalkEffects(posX, posY - 1, posZ);
            } else {
                MinecraftRunner.minecraft.effectRenderer.addWalkEffects(posX, posY, posZ);
            }
        }
        this.cameraYaw += (sqrt_double - this.cameraYaw) * 0.4f;
        this.cameraPitch += (n - this.cameraPitch) * 0.8f;
        if (this.health > 0) {
            final List<Entity> entitiesWithinAABBExcludingEntity = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.expand(1.0, 0.0, 1.0));
            if (entitiesWithinAABBExcludingEntity != null) {
                for (Entity o : entitiesWithinAABBExcludingEntity) {
    ...
    public void dropPlayerItemWithRandomChoice(final ItemStack itemStack, final boolean noPhysics) {
        if (itemStack == null) {
            return;
        }
        final EntityItem entity = new EntityItem(this.worldObj, this.posX, this.posY - 0.30000001192092896, this.posZ, itemStack);
        ...
        if (noPhysics) {
            ...
            entity.motionX += Math.cos(n2) * n;
            entity.motionY += (this.rand.nextFloat() - this.rand.nextFloat()) * 0.1f;
            entity.motionZ += Math.sin(n2) * n;
        }
