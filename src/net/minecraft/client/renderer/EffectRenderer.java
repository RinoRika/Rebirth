...
import net.minecraft.util.RandomUtil;
...
public class EffectRenderer {
    ...
    public void renderParticles(final Entity eq, final float float2) {
        ...
        for (int i = 0; i < 2; ++i) {
            if (!this.fxLayers[i].isEmpty()) {
                int n = 0;
                if (i == 0) {
                    n = this.renderer.getTexture("/particles.png");
                }
                if (i == 1) {
                    n = this.renderer.getTexture("/terrain.png");
                }
    ...
    public void addWalkEffects(double x, double y, double z) {
        final int blockId = this.worldObj.getBlockId(MathHelper.floor_double(x), (int) y - 1, MathHelper.floor_double(z));
        if (blockId == 0) {
            return;
        }
        final Block gs = Block.blocksList[blockId];
        this.addEffect(new EntityDiggingFX(this.worldObj, x + RandomUtil.nextDouble(-0.25, 0.25), (int) y, z + RandomUtil.nextDouble(-0.25, 0.25), 0.0, 0.0, 0.0, gs).multiplyVelocity(0.2f).multipleParticleScaleBy(0.6f));
    }
