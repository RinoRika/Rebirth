...
public class Mipmaps
{
    ...
    private static int ceilPowerOfTwo(int val)
    {
        int i;
    ...
    {
        int i = ceilPowerOfTwo(width);
        int j = ceilPowerOfTwo(height);
        ...
        {
            LogManager.getLogger().warn("Mipmaps not possible (power of 2 dimensions needed), texture: {}, dim: {}x{}", iconName, width, height);
    ...
    public static IntBuffer[] makeMipmapBuffers(Dimension[] mipmapDimensions, int[][] mipmapDatas)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < mipmapDimensions.length; ++i)
            {
                ...
                IntBuffer intbuffer = createDirectIntBuffer(j);
